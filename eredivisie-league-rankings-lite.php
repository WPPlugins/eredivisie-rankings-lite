<?php
/*
Plugin Name: Eredivisie Football Rankings Lite
...
function EREDIVISIESPRLPageURL() {
 $EREDIVISIESPRLpageURL = 'http';
 $EREDIVISIESPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $EREDIVISIESPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $EREDIVISIESPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $EREDIVISIESPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_eredivisiesprlstats() 
{
?>
<h2>Eredivisie Rankings Lite</h2>
<?php eredivisiesprl_stats(); ?>
<?php
}

function eredivisiesprlstats_install()
{
register_sidebar_widget(__('Eredivisie Rankings Lite'), 'widget_eredivisiesprlstats'); 
}
add_action("plugins_loaded", "eredivisiesprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'eredivisiesprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'eredivisiesprl_stats_remove' );

function eredivisiesprl_stats_install() 
{
...
add_option("eredivisiesprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("eredivisiesprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("eredivisiesprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE EREDIVISIE Power Rankings Installation", "Hi\n\nLITE EREDIVISIE Power Rankings Activated at \n\n".EREDIVISIESPRLPageURL()."\n\nEREDIVISIE Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function eredivisiesprl_stats_remove() 
{
/* Deletes the database field */
delete_option('eredivisiesprl_scroll_text_color');
delete_option('eredivisiesprl_scroll_text_color1');
delete_option('eredivisiesprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'eredivisiesprl_stats_admin_menu');

function eredivisiesprl_stats_admin_menu() {

add_options_page('Eredivisie Rankings Lite', 'Eredivisie Rankings Lite Settings', 'administrator', 'eredivisie-power-rankings-lite.php', 'eredivisiesprl_stats_plugin_page');
}
}
function eredivisiesprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('eredivisiesprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('eredivisiesprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('eredivisiesprl_scroll_text_color2'));

$myads = "http://www.ibet.ws/ered_stats_rankings_magpie_lite/ered_stats_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $myads;
$rss = eredivisiesprl_fetch_rss( $url );
...
function eredivisiesprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>Eredivisie Rankings Options Page</h2>
...
<p>
<?php echo eredivisiesprl_stats(); ?>
</p>
<?php echo $myads; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want MORE Stats and Information:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to display advanced and more complete EREDIVISIE Power Rankings.
  ...
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/eredivisie-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/eredivisie-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
...
<p><strong>Soccer / Football / Futbol</strong><br />
  <strong>Premier League rankings</strong> <a href="http://wplatest5.thoseamazingparks.com">Preview</a> | <a href="http://www.ibet.ws/download/eredivisie-rankings.zip">Download Now</a> - Complete Premier League Rankings for all 20 Teams.</p>
